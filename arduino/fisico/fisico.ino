...
void readOBD2() {
  ...
  Serial2.write(0x0C);
  timeout = millis();
  while (Serial2.available() < 4 && millis() - timeout < 20) {
    yield();
  }
  if (Serial2.available() >= 4 && Serial2.read() == 0x41 &&
      Serial2.read() == 0x0C) {
    latest_rpm = ((Serial2.read() << 8) | Serial2.read()) / 4;
  }
  ...
  Serial2.write(0x0D);
  timeout = millis();
  while (Serial2.available() < 3 && millis() - timeout < 20) {
    yield();
  }
  if (Serial2.available() >= 3 && Serial2.read() == 0x41 &&
      Serial2.read() == 0x0D) {
    latest_velocity = Serial2.read();
  }
  while (Serial2.available()) {
    Serial2.read();
  }
